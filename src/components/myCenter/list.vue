 ...
 * @LastEditTime: 2020-12-07 23:34:29
...
<template>
  <div class="content">
    ...
    <el-dialog customClass="details customDialog" :title="curItem.name" :visible.sync="curItemVisible"
      @closed="onClose">
      <div class="">
        <span v-show="Number(userInfo.status) === 6" class="action editor" @click="onEdit">编辑</span>
        <span v-show="Number(userInfo.status) === 6" class="action delete" @click="onDeleteRequirment">删除</span>
        <!-- <p class="status">
        ...
        <div class="detailContent" v-show="isEdit">
          ...
          <div class="text">
            ...
            <label for="uploadFile" class="uploadFileLabel">
              ...
              <input ref="referenceUpload" id="uploadFile" type="file" multiple="multiple"
                @change.self="onUploadFile" />
            </label>
            <div class="filesList editorFileList">
              <p v-for="(file, fileIndex) in filesList" :key="fileIndex">
                <span>{{file.name}}</span>
                <button class="downloadBtn" @click="onDownload(fileIndex)">下载</button>
