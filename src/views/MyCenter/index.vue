...
<template>
  <div class="myCenter" v-loading="loading">
    ...
    <div class="right">
      <div class="r-menu">
        <el-tabs v-model="activeName" @tab-click="handleTabsClick">
          <el-tab-pane label="全部" name="1,2,3,4">
            <listMoudle :list="requirementList" :size="requirementLength" :pageindex="pageindex" :userInfo="userInfo" @pageChange="pageChange" @refreshList="refreshList"></listMoudle>
          </el-tab-pane>
          <el-tab-pane label="已提交" name="1">
            <listMoudle :list="requirementList" :size="requirementLength" :pageindex="pageindex" :userInfo="userInfo" @pageChange="pageChange" @refreshList="refreshList"></listMoudle>
          </el-tab-pane>
          <el-tab-pane label="已审核" name="2" :size="requirementLength" :pageindex="pageindex" :userInfo="userInfo" @pageChange="pageChange" @refreshList="refreshList">
            <listMoudle :list="requirementList"></listMoudle>
          </el-tab-pane>
          <el-tab-pane label="已对接" name="3" :size="requirementLength" :pageindex="pageindex" :userInfo="userInfo" @pageChange="pageChange" @refreshList="refreshList">
            <listMoudle :list="requirementList"></listMoudle>
          </el-tab-pane>
          <el-tab-pane label="已完成" name="4" :size="requirementLength" :pageindex="pageindex" :userInfo="userInfo" @pageChange="pageChange" @refreshList="refreshList">
            <listMoudle :list="requirementList"></listMoudle>
          </el-tab-pane>
        </el-tabs>
        <button v-show="Number(this.userInfo.status) === 6" class="create-requirement" @click="clickCreateRequirement">+
          创建需求</button>
      </div>

      <el-dialog customClass="customDialog" title="创建需求" :visible.sync="createRequirementVisible" width="800px">
        <div>
          <el-form :model="createRequirementForm" :rules="rules" ref="createRequirementForm" label-width="100px" class="demo-ruleForm">
            ...
            <el-form-item label="附件">
              <label for="uploadFile" class="uploadFileLabel">
                ...
                <input id="uploadFile" type="file" multiple="multiple" @change.self="onUploadFile" />
                <ul>
      ...
      <el-dialog customClass="customDialog" title="创建成功" :visible.sync="createSuccessVisible" width="500px" show-close>
